...
if ($evento_id === 0 || ($qty_normal + $qty_jovem + $qty_senior) === 0 || $nome_cli === '' || $email_cli === '') {
    header('Location: ../vendedor.php?erro=dados_invalidos');
    exit;
}
...
if (!$resCap) {
    header('Location: ../vendedor.php?erro=evento_nao_encontrado');
    exit;
}
...
if (($vendidos + $total_pedido) > (int)$resCap['capacidade']) {
    header('Location: ../vendedor.php?erro=sem_lugares&evento_id=' . $evento_id);
    exit;
}
...
$insCompra->bindValue(':nif',    $nif_cli,           SQLITE3_TEXT);
$insCompra->bindValue(':vid',    $vendedor['id'],    SQLITE3_INTEGER);
...
header('Location: ../vendedor.php?sucesso=venda_registada&ref=' . urlencode($referencia));
exit;
